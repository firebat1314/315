<template>
	<div class="classify">
		<my-header1 title="产品分类"></my-header1>
		<div class="nav" style="height: calc(100% - 2.25rem);position: relative;bottom: 0;overflow-y: scroll;">
			<ul class="navl" style="height: 100%;position: relative;bottom: 0;overflow-y: scroll;">
				<li @click="getCData(list.catid)" v-for="(list, index) in TData">
					...
					<div class="nav-child" :id="index" ref="disp" :style="index==0?'display: block;':'display: none;'">
			...
			<div class="content" style="height: 100%;position: relative;bottom: 0;overflow-y: scroll;">
				<div class="content-a" v-for="(data, index) in childData">
					<div v-for="(list, index) in data.list" :style="pcatid1 == data.pcatid1? 'display:block' : 'display:none;'">
						<router-link :to="{path:'/products',query:{cat_id:list.catid}}" style="color: #000;">
						...
						<div class="content-b">
							<ul>
								<li v-for="(child, index) in list.children" :style="index%2!=0?'border-right:0;':((index+1)/list.children.length==1)?'border-bottom:0;border-right:0;':''">
									<router-link :to="{path:'/products',query:{cat_id:child.catid}}" style="color: #959699;">
