...
<script>
	...
	export default {
		...
		methods: {
			setData() {
				this.$http.get(getOpenUrl() + 'applyEdit', {
					headers: {
				...
				.then((res) => {
					if(res.body.code==0){
						this.jibenData=res.body.data
						setCookie("ztitle",this.jibenData.title, expireDays);
						setCookie("ytitle",this.jibenData.ytitle, expireDays);
						setCookie("data",this.jibenData.catid, expireDays);
						setCookie("zzjgdm", this.jibenData.zzjgdm, expireDays);
						setCookie("jyfw", this.jibenData.jinyfw, expireDays);
						setCookie("qyzz",this.jibenData.qiyzz, expireDays);
						setCookie("qyzs", this.jibenData.qiyzs, expireDays);
						setCookie("qyjj", this.jibenData.qiyjj, expireDays);
					}
				}, (error) => {
							if(error.status == 401) {
								this.tips = "用户失效，请登录"
