<template>
	<div class="products">
					...
					<router-link :to="{path:'/pro_details',query:{id:data.id}}" class="chang" v-for="(data,index) in items" :key="index">
						...
						<div class="chang-center" style="position: relative;">
							<ul>
								...
								<li class="changli3">{{data.description}}</li>
								<li class="changli2" v-text="data.price==0?'面议':'￥'+data.price" style="position: absolute;bottom: 0.2rem;"></li>
							</ul>
						</div>
		...
		<mt-popup v-model="popupVisible" position="right" class="all-fenlei">
			<div style="width: 100%;height:calc(100% - 1.2rem);overflow-y:scroll;overflow-x: hidden;padding-top: 0.4rem;">
		...
		<mt-popup v-model="indexlist" position="right" class="all-fenlei" style="padding-top: 0.4rem;">
			...
			<div style="background: #f3f5f7;position:relative;top:0.75rem;bottom:0;font-size: 0.22rem;height: calc(100% - 0.75rem);">
				<swiper :options="swiperOptionCon1" id="swiper-container3" ref="mySwiperCon1">
					<swiper-slide v-for="(list, index) in TData">
						<ul class="navl" style="height: calc(100% - 3rem);overflow-y: scroll;">
						...
						<div class="content" style="height: calc(100% - 3rem);overflow-y: scroll;">
...
//				this.getData(catid, start, size, order, sort,area_id,brand_id,company_id,price_id)
			...
			loadTop() { // 刷新数据的操作
				this.start = 1;
				this.$http.post(getOpenUrl() + "/product_list", {
			...
			getData(catid, start, size, order, sort,area_id,brand_id,company_id,price_id) {
				...
				this.$http.post(getOpenUrl() + "/product_list", data, {
					headers: {
						"Authorization": "Bearer " + getToken()
					}
				}).then((res) => {
